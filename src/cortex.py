...
def ai_complete(prompt: str, model: str, max_tokens: int = 1200) -> str:
	...
	raw_response = df.iloc[0]["RESPONSE"]

	if raw_response is None:
		raise RuntimeError("AI_COMPLETE returned no response.")

	text = str(raw_response).strip()

	# AI_COMPLETE may return a JSON-encoded string containing
	# outer quotes and escaped newline characters.
	try:
